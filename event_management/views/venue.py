...
class VenueCreateView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            address = venue_address.get('address')
            latitude = venue_address.get('latitude')
            longitude = venue_address.get('longitude')
            ...
            venue_obj = Venue.objects.create(
                name=name, address=address_obj, amenities=amenities, capacity=capacity, contact_person=contact_person,
                contact_mobile=contact_mobile
            )

            venue_fields = ['name', 'amenities', 'capacity', 'contact_person', 'contact_mobile']
            json_data.update({'venue': json_formatter(data_obj=venue_obj, fields=venue_fields)})
            status = HTTP_200_OK
        else:
            json_data = {'form': 'invalid'}
            json_data.update({'error': serializer.errors})
